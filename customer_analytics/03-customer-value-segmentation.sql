...
SELECT
	*,
    PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY total_spend) OVER () AS Q1,
    PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY total_spend) OVER () AS Medyan,
    PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY total_spend) OVER () AS Q3
FROM TotalSpends)
,Segments AS 
(
SELECT 
	...
	total_spend,
	CASE
		WHEN total_spend < Q1 THEN 'Düşük Değerli'
		WHEN total_spend BETWEEN Q1 AND Q3 THEN 'Orta Değerli'
		ELSE 'Yüksek Değerli'
	END AS CustomerSegment
FROM quartiles)
SELECT 
    'Müşteri Sayısı' AS [Segment], -- Burası sizin "Index" sütununuz olur
